...
def root(context, missing=missing):
    ...
    template = environment.get_template('pages/exploration_editor/editor_tab/state_editor.html', 'pages/exploration_editor/editor_tab/editor_tab.html')
    ...
    for event in template.root_render_func(template.new_context(context.get_all(), True, {})):
        yield event
    yield u'\n      '
    template = environment.get_template('pages/exploration_editor/editor_tab/issues_overview.html', 'pages/exploration_editor/editor_tab/editor_tab.html')
    for event in template.root_render_func(template.new_context(context.get_all(), True, {})):
        yield event
    yield u'\n    </div>\n  </div>\n  <attribution-guide></attribution-guide>\n</div>'

blocks = {}
debug_info = '3=11&7=15&8=19'